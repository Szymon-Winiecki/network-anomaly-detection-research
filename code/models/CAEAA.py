...
class CAEAA(AEBase, IADModel):
    """ Modified Clustering-based Deep Autoencoder (CAE)
    ...
    And my own idea of using artificial anomalies to improve the model's performance.
    """
    ...
    def __init__(self, 
                 ...
                 artificial_anomalies_ratio : float = 0.5,
                 anomaly_loss_force : float = 2.0) -> None:
        """
        Args:
        ...
        self.clustering_start_epoch = clustering_start_epoch

        self.num_clusters = num_clusters
        self.threshold_quantile = threshold_quantile

        self.artificial_anomalies_ratio = artificial_anomalies_ratio
        self.anomaly_loss_force = anomaly_loss_force
        ...
        self.train_step_anomaly_losses = []
        self.train_step_anomaly_latents = []
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.trainer.current_epoch >= self.clustering_start_epoch and self.choose_centers:
            # in the first batch of the first epoch choose cluseter centers
        ...
        distances_to_the_closest_center, _ = self._calc_clusters(x_latent)
        ...
        if self.trainer.current_epoch < self.clustering_start_epoch:
            return loss.mean()

        self.kde.fit(x.cpu().numpy())
        anomaly_x = self.kde.sample(int(self.artificial_anomalies_ratio * x.shape[0]))
        anomaly_x = torch.tensor(anomaly_x, device=self.device).type_as(x)
        anomaly_x_latent = self.encoder(anomaly_x)
        anomaly_distances_to_the_closest_center, _ = self._calc_clusters(anomaly_x_latent)
        anomaly_loss = self._calc_anomaly_loss(anomaly_distances_to_the_closest_center)

        self.train_step_anomaly_latents.append(anomaly_x_latent.detach().clone())
        self.train_step_anomaly_losses.append(anomaly_loss.detach().clone())

        return loss.mean() + anomaly_loss
    ...
    def on_train_epoch_end(self):
        ...
        anomaly_latents = torch.cat(self.train_step_anomaly_latents)
        anomaly_losses = torch.Tensor(self.train_step_anomaly_losses).to(self.device)
        ...
        self.log("anomaly_loss", anomaly_losses.mean())
        ...
        self.train_step_anomaly_latents.clear()
        self.train_step_anomaly_losses.clear()

        if self.latent_evolution_plot_path and self.trainer.current_epoch % self.latent_evolution_plot_interval == 0:
            self.latent_space_evolution_axs = plot_latent_space_evolution(
                # np.concatenate([latents.detach().cpu().numpy(), anomaly_latents.detach().cpu().numpy()]), 
                latents.detach().cpu().numpy(),
                # np.concatenate([clusters.detach().cpu().numpy(), np.full((anomaly_latents.shape[0]), -1)]),
                clusters.detach().cpu().numpy(),
                axs = self.latent_space_evolution_axs,
                t = self.trainer.current_epoch // self.latent_evolution_plot_interval,
                t_max = self.trainer.max_epochs // self.latent_evolution_plot_interval,
                epoch = self.trainer.current_epoch,
                save_path = self.latent_evolution_plot_path, 
            )
    ...
    def fit(self, 
            ...
            logger_params={},
            random_state=None,
            latent_evolution_plot_path=None,
            latent_evolution_plot_tmax=10):
        

        if latent_evolution_plot_path:
            self.latent_evolution_plot_path = latent_evolution_plot_path
            self.latent_evolution_plot_interval = max(1, max_epochs // latent_evolution_plot_tmax)
            self.latent_space_evolution_axs = None
        else:
            self.latent_evolution_plot_path = None
    ...
    def _calc_loss(self, x : torch.Tensor, x_recon : torch.Tensor, distances_to_the_closest_center : torch.Tensor) -> tuple:
        ...
        if self.trainer.current_epoch < self.clustering_start_epoch:
            return re_loss.mean(), re_loss, torch.tensor([0.0], device=self.device), torch.tensor([0.0], device=self.device)
    ...
    def _calc_anomaly_loss(self, distances_to_the_closest_center : torch.Tensor) -> torch.Tensor:
        centering_loss = 1 / distances_to_the_closest_center

        return self.anomaly_loss_force * centering_loss.mean()
