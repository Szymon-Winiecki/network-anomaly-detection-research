...
class LatentSpaceEvolutionPlotter(Callback):
    ...
    def __init__(self, save_dir, filename, dataset_name, plot_epochs=[], plots_per_row: int = 3):
        """
        Args:
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        self.fig, self.axs = plot_latent_space_evolution(latents, labels, pl_module.name, self.dataset_name, trainer.current_epoch, fig=self.fig, axs=self.axs, t=self.counter, t_max=t_max, plots_per_row=self.plots_per_row, save_path=save_path)
        self.counter += 1
